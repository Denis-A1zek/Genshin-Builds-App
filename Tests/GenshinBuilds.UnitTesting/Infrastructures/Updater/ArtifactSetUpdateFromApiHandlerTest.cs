...
internal class ArtifactSetUpdateFromApiHandlerTest
{
    private Mock<IUnitOfWork> _unitOfWork;
    private Mock<IDataProvider<IEnumerable<ArtifactSet>>> _characterProvider;
    private Mock<IRepository<ArtifactSet>> _characterRepository;
    ...
    public void Setup()
    {
        ...
        _characterProvider = new Mock<IDataProvider<IEnumerable<ArtifactSet>>>();
        _characterRepository = new Mock<IRepository<ArtifactSet>>();
        _characterUpdateHandler = new ArtifactsUpdateFromApiHandler(_unitOfWork.Object, _characterProvider.Object);
    }
    ...
    public async Task UpdateAsync_ShouldReturnUpdateResult_WithDifEqu50()
    {
        ...
        List<ArtifactSet> characterDb = new();
        for (int i = 0; i < countInDb; i++)
            characterDb.Add(fixture.Create<ArtifactSet>());

        List<ArtifactSet> characterInSity = new List<ArtifactSet>(characterDb);
        for (int i = 0; i < newCharacterSize; i++)
            characterInSity.Add(fixture.Create<ArtifactSet>());

        _characterRepository.Setup(r => r.GetAllAsync()).ReturnsAsync(characterDb);
        _unitOfWork.Setup(s => s.GetRepository<ArtifactSet>()).Returns(_characterRepository.Object);
